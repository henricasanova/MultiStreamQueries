...
//			time_t start_time = time (NULL);
			long double order_cost = Cost4Arrays(DNFTree, nb_and, nb_leavesperAND, streamCost,
...
void enumerate_permutations_optim(long double &optimalcost, long double partialcost, int nb_total_leaves, int Order_NbLeavesSet,
	...
	for(int permut_rank=0; permut_rank<nb_permutations[and_id]; permut_rank++){
		...
		if (INDEBUG){
			if(and_rank == nb_and-1){
				order_cost = Cost4Arrays(DNFTree, nb_and, nb_leavesperAND, streamCost,
						nb_total_leaves, Order_AndIds, Order_LeafIds);
			}
		}
...
//			cout << endl;
		...
		for(int leaf_id=0; leaf_id<nb_leavesperAND[and_id]; leaf_id++){
			int stream_id = DNFTree[and_id][leaf_id].stream;
			int nb_data_recup = DNFTree[and_id][leaf_id].nb_data_recup;
...
void enumerate_permutations_singlestream(long double &optimalcost, long double partialcost, //int nb_total_leaves, int Order_NbLeavesSet,
//		int * Order_AndIds, int * Order_LeafIds,
		leaf ** DNFTree, int nb_and, int * nb_leavesperAND, long double * streamCost,
		int * and_permutation, int and_rank,
//		int * nb_permutations, int *** all_the_permutations,
		int nb_streams,
//		int * max_element_per_stream, bool *** AND_needs_data, long double *** proba_data_read,
//		long double * probaAndTrue, bool * andCompleted,
...
//	}
    ...
    multiset<Feuille> myset_feuille1;
    std::vector<multiset<Feuille> > disjoint_set(nb_streams, (myset_feuille1));
//	vector<multiset<Feuille> > disjoint_set = vector<multiset<Feuille> > ();
    for (int i = 0; i < nb_leavesperAND[and_id]; i++){
    	(disjoint_set[DNFTree[and_id][i].stream]).insert(boost::tuples::tuple<int, int, long double, int, int>(DNFTree[and_id][i].nb_data_recup, DNFTree[and_id][i].stream,
    			DNFTree[and_id][i].proba, and_id, i));
    }
//		for (int s = 0; s < nb_streams; s++){
//			if (DNFTree[and_id][i].stream == s){
//				(disjoint_set[s]).insert(boost::tuples::tuple<int, int, long double, int, int>(DNFTree[and_id][i].nb_data_recup, DNFTree[and_id][i].stream,
//						DNFTree[and_id][i].proba, and_id, i));
//			}
//		}
//	}
	int leaf_rest = nb_leavesperAND[and_id];
	long double newpartialcost = partialcost;
	opt_single_and(DNFTree, nb_streams, nb_and, nb_leavesperAND, streamCost,
			leaf_rest, newpartialcost, disjoint_set, final_order);
	if(and_rank == nb_and-1){
		if (newpartialcost<optimalcost){
			optimalcost = newpartialcost;
//			cout << optimalcost << "\t";
//			for (vector<leaf_place>::iterator it0 = final_order.begin(); it0 < final_order.end(); it0++){
//				cout << "(" << it0->get<0> () << ", "
//			                << it0->get<1> () << ")  ";
//			}
//			cout << endl;
...
//		andCompleted[and_id] = true;
		enumerate_permutations_singlestream(optimalcost, newpartialcost,
//				nb_total_leaves,
//				Order_NbLeavesSet+nb_leavesperAND[and_id], Order_AndIds, Order_LeafIds,
				DNFTree, nb_and, nb_leavesperAND, streamCost,
				and_permutation,and_rank+1,
//				nb_permutations, all_the_permutations,
				nb_streams,
//				max_element_per_stream,
//				AND_needs_data, proba_data_read, probaAndTrue, andCompleted,
				final_order);
//		andCompleted[and_id] = false;
	}
                      ...
                      long double & optimal_cost)
{
	...
	// Building all permutations of the ANDs
	allPermutations4And(nb_and, &nb_and_permutation, &permutations_of_ANDs);

	// Data structure to store the order
    vector<leaf_place> evaluation_order;

    //cout << endl << endl << "exhaustive_andbyand_singlestream" << endl << endl;

	optimal_cost = DBL_MAX;
	for(int i=0; i<nb_and_permutation; i++){
		if (WITHPROGRESS){
			cout << endl << i+1 << "/" << nb_and_permutation << ":\t" << flush;
		}
		enumerate_permutations_singlestream(optimal_cost, 0,
				DNFTree, nb_and, nb_leavesperAND, streamCost,
				permutations_of_ANDs[i], 0,
				nb_streams,
				evaluation_order);
	}
